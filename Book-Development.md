...
1. **Language Processing and the Natural Language Toolkit**
    ...
    2. Sub-tasks: WSD, pronoun resolution, entailment, finding things in text, language modeling, collocations, sentence segmentation, lexicon (associating meaning with words, and learning those associations automatically), normalization (stemming, unicode, case), syntax (how do different words in the sentence relate to one another; e.g., agent of a verb), named entity recognition? -- (identify tasks and writers by mid June)
